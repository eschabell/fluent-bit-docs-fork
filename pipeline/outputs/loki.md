...
| `oauth2.client_secret` | Required when `oauth2.auth_method` is `basic` or `post`. `OAuth 2.0` client secret. | _none_ |
| `oauth2.connect_timeout` | Connect timeout for `OAuth 2.0` token requests. | `0s` |
| `oauth2.enable` | Enable `OAuth 2.0` client credentials for outgoing requests. See the `OAuth 2.0` authentication section. | `false` |
...
| `oauth2.jwt_cert_file` | Required when `oauth2.auth_method` is `private_key_jwt`. Path to certificate file used by `private_key_jwt`. | _none_ |
| `oauth2.jwt_header` | JWT header claim name for the `private_key_jwt` certificate thumbprint. Recognized values are `kid` (hexadecimal SHA-1), `x5t` (base64url SHA-1), and `x5t#S256` (base64url SHA-256). Any other value is used as the claim name with a hexadecimal SHA-1 thumbprint. | `kid` |
| `oauth2.jwt_key_file` | Required when `oauth2.auth_method` is `private_key_jwt`. Path to PEM private key file used by `private_key_jwt`. | _none_ |
